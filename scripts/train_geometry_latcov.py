"""Train the geometry-aware SoS flow with latent-covariance FM paths (v6B).

Same data, encoder, backbone and sampling as train_geometry_flow; only the
training path changes: u_t and the velocity target follow
Sigma_t = [(1-t)I + t*alpha*Gamma_i^{1/2}]^2 with Gamma_i the k-NN local
covariance of the target's SoS-latent patches (see models/latent_cov.py).
"""
...
from models.latent_cov import LatentCovarianceNoise
...
def parse_args():
    ...
    p.add_argument('--bank', default='/data/zhuangyang/geometry_flow_v2_patchbank/patch_bank.pt')
    p.add_argument('--lc-k', type=int, default=32)
    p.add_argument('--lc-rank', type=int, default=8)
    p.add_argument('--lc-alpha', type=float, default=.5)
...
def main():
    ...
    log(f'latent-cov FM: k={args.lc_k} rank={args.lc_rank} alpha={args.lc_alpha} '
        f'bank={args.bank}')
    ...
    latcov = LatentCovarianceNoise(args.bank, k=args.lc_k,
                                   rank=args.lc_rank,
                                   alpha=args.lc_alpha).to(device)
    for epoch in range(1, epochs+1):
        ...
        for batch in train_loader:
            b = move(batch, device)
            model.flow._update_u_scale(b['u_gt'])
            t = model.flow._sample_time(b['u_gt'].shape[0], device)
            with torch.no_grad():
                u_t, v_star = latcov(b['u_gt'], t, model.flow._u_scale)
            encoded = model.encoder(b['condition'])
            v_pred = model.flow.net(torch.cat([encoded, u_t], dim=1), t)
            loss = torch.nn.functional.mse_loss(v_pred, v_star)
            if not torch.isfinite(loss):
                raise FloatingPointError('Nonfinite latent-cov training loss')
