...
def main():
    ...
    for path in sorted(args.raw_dir.glob('*.npz')):
        ...
        shard = {'rf': torch.from_numpy(d['rf']), 'c': torch.from_numpy(d['c']),
                 ...
                 'metadata': {
                     'id': meta['id'], 'split': 'train', 'case': meta['case'],
                     'backend': 'ultrawave',
                     'base_anatomy_id': f"{meta['case']}/z{meta['z_index']}_"
                                        f"{meta['variant']}",
                     'scatter_seed': meta['scatter_seed'],
                     ...
                     'preset': 'dual_scale',
                     'simulation': ('decoupled counterfactual '
                                    f"({meta['variant']})")}}
        torch.save(shard, args.out_root/'shards'/f"{meta['id']}.pt")
        samples.append({'id': meta['id'], 'split': 'train',
                        'backend': 'ultrawave', 'variant': meta['variant'],
